...
int main()
{
    ...
    hDir = CreateFile(  L"D:\\Code\\심화과정\\WindowPrograming",
                        FILE_LIST_DIRECTORY,
    ...
    for (int i = 0; i < 10; i++) {
        ...
        /// ReadDirectoryChangesW 는 현재 위치에서 Pendding (대기) 상태로 변환
        ...
        {
            printf("ReadDirectoryChangesW : %d\n", GetLastError());
            CloseHandle(hDir);
            return -9;
        }
        ...
        while (TRUE)
        {
            ...
            printf("%d\nNextEntryOffset [ %d ] Action [ %d ] FNL [ %d ] FN [ %ws ]\n",
                i, event->NextEntryOffset, event->Action, event->FileNameLength, event->FileName);
