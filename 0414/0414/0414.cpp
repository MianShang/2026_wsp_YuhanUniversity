...
void lit() 
{
    ...
    printf("LOW[ %d ] HI[ %d ]\n", litest.LowPart, litest.HighPart);
    printf("QUAD[ %lld ]\n", litest.QuadPart);
    // 오버플로우 발생 -> 정보의 손실이 발생해서 199 까지만 나옴
   // int 형식의 32bit 시스템에서는 하위 값만 보관 
...
int main()
{
    ...
    for (int i = 0; i < 45; i++) {
        ...
        liMove.QuadPart = i;
        /// 파일의 시작 위치로 파일의 포인터를 이동시켜라.
        if (FALSE == SetFilePointerEx(h, liMove, &liAfter, FILE_BEGIN))
        {
            printf("파일 이동 실패 : %d", GetLastError());
            CloseHandle(h);
            return -9;
        }
        ...
        {
            printf("파일 읽기 실패 : %d", GetLastError());
            CloseHandle(h);
            return -9;
        }
